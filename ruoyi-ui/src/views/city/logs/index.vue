...
<script>
import { listLogs, getLogs, delLogs, addLogs, updateLogs } from "@/api/city/logs";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除工单操作日志编号为"' + ids + '"的数据项？').then(function() {
        return delLogs(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('city/logs/export', {
        ...this.queryParams
